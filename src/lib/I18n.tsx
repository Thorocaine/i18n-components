...
function useTranslation(args: any[], index: string) {
  ...
  useEffect(
    () => {
      const template =
        (config[locale] && config[locale][index]) ||
        (config[locale.split('-')[0]] && config[locale.split('-')[0]][index]) ||
        indexNotFound(index);
      const text = args.reduce(
        (all, val, i) => all.split(`\$\{${i}\}`).join(val),
        template
      );
      setTranslation(text);
    },
...
const TranslationComp = ({ args, index }: { args: any[]; index: string }) => (
  <>{useTranslation(args, index)}</>
);

function i18n(strings: TemplateStringsArray, ...args: any[]) {
  const index = strings.reduce(
    (all, val, i) => `${all}${val}${args.length > i ? `\$\{${i}\}` : ''}`,
    ''
  );
